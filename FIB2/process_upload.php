...
if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
    ...
    if (move_uploaded_file($image_tmp_name, $upload_dir . $image_name)) {
        ...
        if ($conn->query($sql) === TRUE) {
            echo "<script>alert('Belonging uploaded successfully!'); window.location.href='dashboard.php';</script>";
        } else {
            echo "<script>alert('Error: " . $conn->error . "'); window.location.href='upload_belongings.php';</script>";
        }
    } else {
        echo "<script>alert('Failed to upload the image.'); window.location.href='upload_belongings.php';</script>";
    }
} else {
    echo "<script>alert('No image uploaded or there was an error.'); window.location.href='upload_belongings.php';</script>";
}
?>
